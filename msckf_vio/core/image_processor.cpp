...
//类比雨ros ImageProcessor::initialize() 
    ...
    const std::deque<Sensor_imu_t>& imu_buffer)
{
    ...
    //将前一帧的左目特征变换到当前帧左目，光流跟踪前一帧左-当前帧左
...
/*步骤：
1.计算左右cam的位姿变换R10
2.cam0的点（存在畸变）——去畸变（投影到归一化平面再去畸变）——R10——投影cam1图像平面（无畸变）；
3.计算右目cam1的畸变坐标；
4.左右目光流跟踪获得inliers
5.极线校正，更新inliers：计算E10=t^R10,极线方程校正
*/
...
void ImageProcessor::addNewFeatures()
{
    ...
    // Find the stereo matched points for the newly
    // detected features.
    //c.进行stereomatch获得右目特征点，并统计内点，将其划分到对应网格并排序
    std::vector<cv::Point2f> cam0_points(new_features.size());
    for (uint32_t i = 0; i < new_features.size(); ++i) {
        cam0_points[i] = new_features[i].pt;
    }
    ...
    std::vector<int32_t>& inlier_markers)
{
    ...
    for (uint32_t i = 0; i < pts_diff.size(); ++i) {
        ...
        //一般运动25像素距离即可用于判定，如果是剧烈运动的话，需要增大像素距离
    ...
    for (int32_t iter_idx = 0; iter_idx < iter_num; ++iter_idx) {
        ...
        Eigen::Vector3d model(0.0, 0.0, 0.0);  //为求解的(tx,ty,tz)
        if (base_indicator == 0) {
            Eigen::Matrix2d A;
            A << coeff_ty, coeff_tz;
