...
void MsckfVio::processModel(
    const uint64_t& time,
    const Eigen::Vector3d& m_gyro,   //表示测量值
    const Eigen::Vector3d& m_acc)
{
    // Remove the bias from the measured gyro and acceleration，从测量中计算（理想值）
    IMUState& imu_state = state_server.imu_state;
    Eigen::Vector3d gyro = m_gyro - imu_state.gyro_bias;   //估计值=测量-bg
...
/*状态扩增
1.根据R_i_c（相机-IMU位姿）和R_w_i（IMU-世界位姿）计算当前相机位姿，加入到当前相机的状态向量
...
/*
    1.计算feature在当前camera_state_id下的投影残差；
    2.计算单个feature在单个相机下的雅可比，并修正
*/
    ...
    Eigen::Vector4d& r)
{
    ...
    u.block<3, 1>(3, 0) =
        ...
        IMUState::gravity;
    H_x = A - A * u * (u.transpose() * u).inverse() * u.transpose();  //式（5.23）
    ...
    Eigen::VectorXd& r)
{
    ...
    //投影到H_fj的左零空间后，H_x的大小 （4M-3）*（21+6N）  式（4.20）
    ...
    const uint32_t& dof)
{
    ...
    //(P1 + P2).ldlt().solve(r) 表示 （P1+P2）x=r
...
*/
    ...
    for (const auto& feature_id : processed_feature_ids) {
        ...
        Eigen::MatrixXd H_xj;   //（4M-3）*（21+6N）
        Eigen::VectorXd r_j;
        //一个特征被多个相机帧观测
...
/*
相机状态删除：[0,n]帧中主要删除前两帧 0,1 或者n-1 n-2帧
